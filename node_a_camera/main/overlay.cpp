...
void overlay_update_boxes(const tracked_object_t *tracks, int count)
{
    ...
    for (int t = 0; t < count && (int)g_ovl_box_count < MAX_OVL_BOXES; t++) {
        // Instant hide: if the model hasn't seen this track for >1 frame,
        // don't draw its box.  The track stays alive in memory (for re-match)
        // but the overlay won't show a frozen "ghost" rectangle.
        if (tracks[t].frames_since_seen > 1) continue;
...
void draw_overlay(uint8_t *buf, int fw, int fh)
{
    ...
    // Draw a single dashed yellow counting line
    int line_y = oy + ((COUNT_LINE_Y * sc) >> FP);
    for (int x = 0; x < fw; x++) {
        // dash pattern: 8 px on, 4 px off
        uint16_t col = ((x / 8) % 2 == 0) ? 0xFFE0u : 0x0000u;
        set_pixel(buf, fw, fh, x, line_y,     col);
        set_pixel(buf, fw, fh, x, line_y + 1, col);
    }
}
